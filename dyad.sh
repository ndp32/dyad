...
# --- Write API key to file (avoids exposure in process table / ps aux) ---
API_KEY_FILE="${TMPDIR_DYAD}/api-key"
printf '%s' "$RESOLVED_API_KEY" > "$API_KEY_FILE"
chmod 600 "$API_KEY_FILE"

# --- Write hook settings ---
HOOK_CMD="DYAD_TASK_FILE='${TASK_FILE}' DYAD_RULES_FILE='${RULES_FILE}' DYAD_APPROVE_ALL='${APPROVE_ALL}' DYAD_SESSION_ID='${SESSION_ID}' DYAD_PROJECT_ROOT='${PROJECT_ROOT}' DYAD_SESSION_TMPDIR='${TMPDIR_DYAD}' DYAD_API_KEY_FILE='${API_KEY_FILE}' '${HOOK_SCRIPT}'"
